#!/bin/bash
# Launch GRPO v2 on 5x H200: GPU 0 → vLLM, GPUs 1-4 → DDP
set -e
MAX_STEPS=${1:-50}
LR=${2:-5e-6}
RUN_NAME=${3:-grpo_v2_easy}
SAVE_STEPS=${4:-10}
MODEL="deepseek-ai/DeepSeek-R1-Distill-Qwen-14B"
LOG_DIR="outputs/${RUN_NAME}"

# Activate venv (container disk or workspace)
if [ -d /root/venv ]; then
    source /root/venv/bin/activate
else
    source /workspace/rl-chess-training/.venv/bin/activate
fi
...
echo "=== DDP training on GPUs 1,2,3,4 ==="
